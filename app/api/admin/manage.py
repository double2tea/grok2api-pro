...
def _resolve_full_sso(log) -> Optional[str]:
    """为历史日志尽量解析完整SSO（不写入日志文件）"""
    if getattr(log, "sso_full", ""):
        return log.sso_full
    masked = getattr(log, "sso", "") or ""
    prefix = masked.replace("****", "")
    if len(prefix) < 3:
    ...
    try:
        ...
        for token_type_data in all_tokens.values():
            for token in token_type_data.keys():
...
def calculate_token_stats(tokens: Dict[str, Any], token_type: str) -> Dict[str, int]:
    """计算Token统计"""
    total = len(tokens)
    status_counts = {"未使用": 0, "限流中": 0, "失效": 0, "正常": 0}
    for t in tokens.values():
        status = get_token_status(t, "sso" if token_type == "normal" else "ssoSuper")
        if status in status_counts:
            status_counts[status] += 1

    return {
        "total": total,
        "unused": status_counts["未使用"],
        "limited": status_counts["限流中"],
        "expired": status_counts["失效"],
        "active": status_counts["正常"],
    }
...
def get_token_status(token_data: Dict[str, Any], token_type: str) -> str:
    ...
    if relevant == -1:
        return "失效" if token_data.get("status") == "expired" else "未使用"
    elif relevant == 0:
        return "限流中"
    else:
        return "正常"
...
async def list_tokens(
    ...
    try:
        ...
        # 过滤
        if token_type in ["sso", "ssoSuper"]:
            token_list = [t for t in token_list if t.token_type == token_type]
        if status:
            token_list = [t for t in token_list if t.status == status]
        if tag:
            token_list = [t for t in token_list if tag in (t.tags or [])]

        total_count = len(token_list)
        if sort == "created_desc":
            token_list.sort(key=lambda t: t.created_time or 0, reverse=True)
        elif sort == "created_asc":
            token_list.sort(key=lambda t: t.created_time or 0)

        # 兼容旧limit参数（优先）
        if limit is not None and limit > 0:
            token_list = token_list[:limit]
            return TokenListResponse(success=True, data=token_list, total=total_count)
        ...
        token_list = token_list[start_idx:end_idx]

        logger.debug(f"[Admin] Token列表获取成功: {len(token_list)}个")
        return TokenListResponse(success=True, data=token_list, total=total_count, page=page, page_size=page_size)
...
async def get_stats(_: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    ...
    try:
        ...
        # 获取视频统计
        video_stats = token_manager.get_video_stats()
    ...
    try:
        ...
        log_list = [
            LogInfo(
                ...
                sso=log.sso,
                sso_full=_resolve_full_sso(log),
                model=log.model,
                success=log.success,
        ...
        logger.debug(f"[Admin] 日志查询成功: {len(log_list)}/{total}")
        return {
            "success": True,
            "data": log_list,
            "total": total,
        ...
        logger.error(f"[Admin] 查询日志异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"查询失败: {e}", "code": "LOG_QUERY_ERROR"})
...
async def get_log_stats(_: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """获取日志统计信息"""
    try:
        logger.debug("[Admin] 获取日志统计")
        stats = call_log_service.get_stats()
        return {"success": True, "data": stats}
    except Exception as e:
        logger.error(f"[Admin] 获取日志统计异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"获取失败: {e}", "code": "LOG_STATS_ERROR"})
...
async def get_log_models(_: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """获取日志模型列表"""
    try:
        logger.debug("[Admin] 获取日志模型列表")
        models = call_log_service.get_models()
        return {"success": True, "data": models}
    except Exception as e:
        logger.error(f"[Admin] 获取日志模型列表异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"获取失败: {e}", "code": "LOG_MODELS_ERROR"})
...
async def clear_logs(
    ...
    """清理日志"""
    try:
        logger.debug(f"[Admin] 清理日志: max_count={max_count}")
        ...
        logger.debug(f"[Admin] 日志清理完成: 删除 {deleted} 条")
        return {"success": True, "message": f"成功清理 {deleted} 条日志", "deleted_count": deleted}
    
    except Exception as e:
        logger.error(f"[Admin] 清理日志异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"清理失败: {e}", "code": "LOG_CLEAR_ERROR"})
...
class UnassignProxyRequest(BaseModel):
    sso: str


@router.get("/api/proxies")
async def get_proxies(_: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """获取代理列表"""
    try:
        logger.debug("[Admin] 获取代理列表")
        proxies = proxy_pool.get_all_proxies()
        assignments = proxy_pool.get_sso_assignments()
        ...
        logger.error(f"[Admin] 获取代理列表异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"获取失败: {e}", "code": "PROXY_LIST_ERROR"})


@router.post("/api/proxies")
async def add_proxy(request: AddProxyRequest, _: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """添加代理"""
    try:
        logger.debug(f"[Admin] 添加代理: {request.url}")
        
        success = proxy_pool.add_proxy(request.url)
        if success:
            return {"success": True, "message": "代理添加成功"}
        else:
            return {"success": False, "message": "代理格式无效或已存在"}
    
    except Exception as e:
        logger.error(f"[Admin] 添加代理异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"添加失败: {e}", "code": "PROXY_ADD_ERROR"})


@router.delete("/api/proxies")
async def remove_proxy(request: RemoveProxyRequest, _: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """删除代理"""
    try:
        logger.debug(f"[Admin] 删除代理: {request.url}")
        
        success = proxy_pool.remove_proxy(request.url)
        if success:
        ...
        logger.error(f"[Admin] 删除代理异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"删除失败: {e}", "code": "PROXY_REMOVE_ERROR"})


@router.post("/api/proxies/assign")
async def assign_proxy(request: AssignProxyRequest, _: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """绑定代理到SSO"""
    try:
        logger.debug(f"[Admin] 绑定代理: {request.sso[:10]}... -> {request.proxy_url}")
        
        success = proxy_pool.assign_to_sso(request.proxy_url, request.sso)
        if success:
        ...
        logger.error(f"[Admin] 绑定代理异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"绑定失败: {e}", "code": "PROXY_ASSIGN_ERROR"})


@router.post("/api/proxies/unassign")
async def unassign_proxy(request: UnassignProxyRequest, _: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """解绑SSO的代理"""
    try:
        logger.debug(f"[Admin] 解绑代理: {request.sso[:10]}...")
        
        success = proxy_pool.unassign_from_sso(request.sso)
        if success:
            return {"success": True, "message": "解绑成功"}
        else:
            return {"success": False, "message": "SSO未绑定代理"}
    
    except Exception as e:
        logger.error(f"[Admin] 解绑代理异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"解绑失败: {e}", "code": "PROXY_UNASSIGN_ERROR"})


@router.post("/api/proxies/health/reset")
async def reset_proxy_health(request: AddProxyRequest, _: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """重置代理健康状态"""
    try:
        logger.debug(f"[Admin] 重置代理健康状态: {request.url}")
        
        proxy_pool.mark_success(request.url)
        return {"success": True, "message": "健康状态已重置"}
    
    except Exception as e:
        logger.error(f"[Admin] 重置健康状态异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"重置失败: {e}", "code": "PROXY_HEALTH_RESET_ERROR"})


@router.post("/api/proxies/test")
async def test_proxy(request: AddProxyRequest, _: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """测试代理连通性"""
    import aiohttp
    import time
    
    try:
        logger.debug(f"[Admin] 测试代理: {request.url}")
        
        # 测试目标URL
        test_url = "https://grok.com"
        start_time = time.time()
        
        timeout = aiohttp.ClientTimeout(total=10)
        
        # 根据代理类型选择connector
        proxy_url = request.url.strip()
        
        # aiohttp_socks 不支持 socks5h://，需要转换为 socks5://
        if proxy_url.startswith('socks5h://'):
            proxy_url = proxy_url.replace('socks5h://', 'socks5://', 1)
        
        try:
            if proxy_url.startswith(('socks5://', 'socks4://')):
                # SOCKS5代理需要使用aiohttp_socks
                        ...
                        if response.status == 200 or response.status == 403:
                            ...
                            return {
                                ...
                                "data": {
                                    ...
                                    "note": "403表示被CF拦截，但代理本身可用" if response.status == 403 else ""
                                }
                            }
                        else:
                            return {
                                "success": False,
                                "message": f"代理返回异常状态码: {response.status}",
                                "data": {"status_code": response.status, "response_time": elapsed}
                            }
            else:
                # HTTP/HTTPS代理
            ...
            if "timeout" in error_msg.lower():
                return {"success": False, "message": "代理连接超时", "data": {"response_time": elapsed, "error": error_msg}}
            elif "refused" in error_msg.lower():
                return {"success": False, "message": "代理连接被拒绝，请检查代理地址和端口", "data": {"error": error_msg}}
            elif "resolve" in error_msg.lower() or "getaddrinfo" in error_msg.lower():
                return {"success": False, "message": "无法解析代理地址", "data": {"error": error_msg}}
            else:
                return {"success": False, "message": f"代理连接失败: {error_msg}", "data": {"response_time": elapsed}}
    
    except Exception as e:
        logger.error(f"[Admin] 测试代理异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"测试失败: {e}", "code": "PROXY_TEST_ERROR"})
