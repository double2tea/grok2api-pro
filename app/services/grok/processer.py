...
class GrokResponseProcessor:
    ...
    @staticmethod
    async def process_normal(response, auth_token: str, model: str = None) -> Tuple[OpenAIChatCompletionResponse, list]:
        """处理非流式响应"""
            ...
            for chunk in response.iter_lines():
                ...
                if video_resp := grok_resp.get("streamingVideoGenerationResponse"):
                    if video_url := video_resp.get("videoUrl"):
                        media_urls = [f"https://assets.grok.com/{video_url}" if not str(video_url).startswith("http") else video_url]
                ...
                if images := model_response.get("generatedImageUrls"):
                    media_urls = [
                        img if str(img).startswith("http") else f"https://assets.grok.com/{img}"
                        for img in images
                    ]
