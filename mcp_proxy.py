...
GROK_API_KEY = os.getenv("GROK_API_KEY", os.getenv("API_KEY", ""))
...
async def _proxy(request: Request, path: str):
    ...
    if GROK_API_KEY:
        upstream_headers["Authorization"] = f"Bearer {GROK_API_KEY}"
    async with httpx.AsyncClient(timeout=120) as client:
        if request.method == "GET":
            response = await client.get(
                target_url, params=request.query_params, headers=upstream_headers
            )
        else:
...
async def list_models(request: Request):
    ...
    return Response(
        content=content, status_code=status, media_type=headers.get("content-type")
    )
